...
remote_api.get_tardis_by_entity = function(entity)
    if entity == nil then return nil end
    return storage.factories_by_entity[entity.unit_number]
end

remote_api.get_tardis_by_building = function(entity)
    local tardis = storage.factories_by_entity[entity.unit_number]
    if tardis == nil then
        game.print("ERROR: Unbound tardis building: " .. entity.name .. "@" .. entity.surface.name .. "(" .. entity.position.x .. ", " .. entity.position.y .. ")")
    end
    return tardis
end

remote_api.find_tardis_by_area = function(params)
    local surface = params.surface
    local position = params.position
    local area = params.area

    for _, entity in pairs(surface.find_entities_filtered {position = position, area = area, type = BUILDING_TYPE}) do
        if has_layout(entity.name) then return remote_api.get_tardis_by_building(entity) end
    end
    return nil
end
...
remote_api.find_factories_by_area = function(params)
    ...
    for _, entity in pairs(surface.find_entities_filtered {area = area, type = BUILDING_TYPE}) do
        if has_layout(entity.name) then
            local tardis = remote_api.get_tardis_by_building(entity)
            if tardis then factories[#factories + 1] = tardis end
        end
    end
    return factories
end

remote_api.find_surrounding_tardis = function(surface, position)
...
remote_api.find_surrounding_tardis_by_surface_index = function(surface_index, position)
...
remote_api.is_tardis_surface = function(surface)
...
remote.add_interface("tardis", remote_api)
