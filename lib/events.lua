...
---@param f function
tardis.on_event = function(event, f)
...
tardis.on_nth_tick = function(event, f)
    events[event] = events[event] or {}
    table.insert(events[event], f)
end
...
tardis.finalize_events = function()
    ...
    for event, functions in pairs(events) do
        ...
        elseif event == tardis.events.on_init() then
            script.on_init(f)
...
for event, _ in pairs(gui_events) do
    tardis.on_event(event, process_gui_event)
end

local delayed_functions = {}
---use this to execute a script after a delay
---example:
---tardis.register_delayed_function('my_delayed_func', function(param1, param2, param3) ... end)
---tardis.execute_later('my_delayed_func', 60, param1, param2, param3)
---The above code will execute my_delayed_func after waiting for 60 ticks
---@param function_key string
---@param ticks integer
---@param ... any
function tardis.execute_later(function_key, ticks, ...)
...
tardis.on_event(defines.events.on_object_destroyed, function(event)
...
function tardis.register_delayed_function(key, func)
