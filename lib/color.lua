...
tardis.light_tints = {}
for i, tint in pairs(tardis.tints) do
    tardis.light_tints[i] = {}
    for color, amount in pairs(tint) do
        tardis.light_tints[i][color] = (amount - 0.5) / 2 + 0.5
    end
    tardis.light_tints[i].a = 1
end

---@param color Color
---@return Color
function tardis.color_normalize(color)
...
---@return Color
function tardis.color_combine(a, b, percent)
    a = tardis.color_normalize(a)
    b = tardis.color_normalize(b)
