...
tardis.on_event = function() end

---Returns a 1x1 empty image.
---@return table
tardis.empty_image = function()
    return {
...
tardis.merge = function(old, new)
    if not old then
        error("Failed to tardis.merge: Old prototype is nil", 2)
    end
...
tardis.surface_conditions = function()
    return {{
        property = "pressure",
        min = 200000,
        max = 400000,
...
tardis.shorten_localised_string = function(localised_string)
    ...
    return {"", tardis.shorten_localised_string(first_half), tardis.shorten_localised_string(second_half)}
end
